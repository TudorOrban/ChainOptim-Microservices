#!/bin/bash

# Default value for rd
rd="false"

# Parsing command line arguments for 'rd'
while getopts ":r:" opt; do
  case $opt in
    r) rd="$OPTARG"
    ;;
    \?) echo "Invalid option -$OPTARG" >&2
    ;;
  esac
done

# Navigate to parent directory
pushd ..

# Display the current directory
echo "Current directory: $(pwd)"

# Build the Notifications project (skipping tests)
echo "Building the Notifications project (skipping tests)..."
mvn clean package -DskipTests

# Build Notifications Docker image
echo "Building Notifications Image..."
docker build -t tudoraorban/chainoptim-notifications:latest .

# Upload Notifications Docker image to DockerHub
echo "Upload Notifications Image to DockerHub..."
docker push tudoraorban/chainoptim-notifications:latest

# If rd is true, restart the deployment
if [ "$rd" = "true" ]; then
    echo "Restarting the Notifications deployment..."
    kubectl rollout restart deployment chainoptim-notifications
fi

# Return to the original directory
popd
